...
globle struct joinNode *ConstructJoins(
  ...
  int startDepth)
  {
   ...
   intBool useLinks;
#if SLIDING_WINDOW
   struct deftemplate* template = NULL;
#endif
...
#if SLIDING_WINDOW
		 //printf("thePattern:%s %s\n", theLHS->right->value, ValueToString(theLHS->right->bottom->value));
		 template = EnvFindDeftemplate(theEnv, ValueToString(theLHS->right->bottom->value));
#endif
...
#if SLIDING_WINDOW
		 lastJoin->numOfTmp = template->numOfBeta++;
		 //if (strcmp(template->header.name->contents, "cTN64") == 0)
		 //{
		//	 printf("depth %d %d\n", lastJoin->depth,lastJoin->numOfTmp);
		 //}
#endif
        }
